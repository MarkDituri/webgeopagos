<?php
require_once("Models/TLoginMenu.php");
...
{
	use TLoginMenu, TRestaurante, TCarrito, TSliders, TCategoria, TProducto, TCliente;
	// public $login;
	...
	public function id($params)
	{
		if (empty($params)) {
			...
			if (empty($data['restaurante'])) {
				header("Location:" . base_url() . '/menu/error/?msg=sinrest');
			} else if($data['ultimoPago']['status'] == 'vencido') {
				header("Location:" . base_url() . '/menu/error/?msg=nopay');
			}				
			// LOGIN CON RESTAURANTE 	
			$this->loginUserMenu($id_restaurante);			
	...
	{
		$sesion = $this->sesionMenu();		
		if (!$sesion) {
			header("Location:" . base_url() . '/menu/error');
		}		
		$this->getColor($sesion['restaurante']['id_color']);	
		if($_SESSION['carrito']['status'] == 1){	
			header("Location:" . base_url() . '/menu/id/' . $_SESSION['restaurante']['url']);
		} 
	...
	public function delproducto($params)
	{
		...
		if ($responseDelCarrito) {
			header("Location:" . base_url() . '/menu/carrito');
		}
		die();    
	}
	...
	public function resetmenu()
	{
		...
		$sesionOut = $this->sesionMenuReset();
		if ($sesionOut) {
			header("Location:" . base_url() . '/menu/id/'.$url_restaurante);
		}	
		die();
	}
	...
	{		
		...
		$url_restaurante = $_SESSION['restaurante']['url'];
		$sesionOut = $this->sesionMenuCerrar();
		if ($sesionOut) {
			header("Location:" . base_url() . '/menu/id/'.$url_restaurante);
		}	
		die();
